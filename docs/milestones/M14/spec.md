...
| `tests/test_channel_neutrality.py` | one Tool declared for both channels yields the same output through both; the same Tool with `channels={AGENT}` runs through an Agent ToolRuntime window and is refused through a Web one, with the handler untouched; what each adapter does with that exposure is proven in `tests/test_mcp_adapter.py` and `tests/test_nicegui_adapter.py` |
...
| `tests/test_invoke_command.py` | `--channel`, `--principal` required (argparse exit 2 with usage); against `expense-app`: `approve_expense` as `alice` is one stderr report `tool.forbidden` `role_required` and exit 1; with `--role manager` the gate passes and the command exits 0 with the App's own `expense.not_found`, because each `invoke` process opens a fresh ledger and the command cannot reach an existing expense; the two levels over a live ledger are proven in-process by `tests/test_tool_authorization.py::test_expense_shows_the_two_levels_of_authorization`; `--channel web` on `measure_note` still runs (default `channels`) |
