...
(`vibepy_core.app._record`, reached as `vibepy_core.app`). The lifespan's exit has to be visible
from outside the process, and the framework already has the contract for that — a JSON record per line on standard error — so closing
