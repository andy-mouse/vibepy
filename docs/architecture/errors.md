...
`app.declaration_invalid` carries its members: every Tool-name conflict, Page-name conflict,
invalid route, route conflict, and Page→Tool reference that is missing or not exposed on the Web
channel, found constructing one `AppDefinition`. `report` writes it as one line for the aggregate
and then one line per member, so a declaration invalid several ways over is several lines rather
than one that hides the rest. `authoring.type_error` is Studio's own vocabulary, defined in
`vibepy_studio/authoring/models.py`, not here.
