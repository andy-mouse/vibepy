...
not satisfy the Protocol written to describe it, and a private `_ToolRuntimeInvoker` exists whose
...
second Protocol, `ToolInvocation`, was introduced to hold the spread back. That treats the symptom: the Page
...
- the Page package imports nothing from the Tool package, which is stronger than before this
  decision
- a Page calls `ctx.tools.invoke(name, raw_input)`. Page code that called `ctx.tools.call`
  must be updated; the framework is pre-release and no
