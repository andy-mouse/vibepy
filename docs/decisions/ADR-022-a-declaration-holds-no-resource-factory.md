# ADR-022: A declaration holds no resource factory
...
validation, plugin inspection and conformance validation all depend on that, which is the
...
assembling something, not a statement about what the plugin is. Every other field can be read,
projected and validated; this one can only be called. A reader that wants to know what a plugin
declares learns nothing from it, and a reader that wants to know what resource it needs learns
nothing either, because `DepsT` is a type the plugin chose and the framework only carries.
...
(<https://blog.ploeh.dk/2011/07/28/CompositionRoot/>). A Plugin's entrypoint is such an
application: it is the one place that knows both which plugin this is and what its resource is
made of.

## Decision

`PluginDefinition` declares no resource factory. It declares identity, Tools and Pages.
...
- what a plugin may declare *about* its host — configuration schema, secret names, required
