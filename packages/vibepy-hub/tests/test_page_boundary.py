...
boundary at the import level: no module under `vibepy_hub/pages/` may import
`vibepy_hub.internals`, `vibepy_hub.tools`, or `vibepy_core.tool` — a Page's whole reach into
Hub Core is `ctx.tools.invoke`.
"""
...
PAGES = Path(__file__).resolve().parents[1] / "src" / "vibepy_hub" / "pages"
